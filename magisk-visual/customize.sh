...
ui_print "- TS18 Compact Status Bar Visuals"
ui_print "- Validating exact target and independent SystemUI RRO payload"
...
apk="$MODPATH/system/product/overlay/TS18StatusBarVisuals.apk"
if [ ! -s "$apk" ]; then
    abort "STOP: missing or empty SystemUI visual RRO payload: $apk"
fi

set_perm "$apk" 0 0 0644
ui_print "- No OEM APK, layout or RRO is deleted or replaced"
ui_print "- Reboot required"
ui_print "- If idmap rejects the overlay, disable this module and retain stock visuals"
